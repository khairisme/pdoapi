...
namespace HR.PDO.Application.Services.PDO
{
                ...
                var query = await (from a in _dbContext.PDOSkimPerkhidmatan
                                   ...
                                   select new
                                   {
                                       ...
                                       GredList = (from g in _dbContext.PDOGredSkimPerkhidmatan
                                                   where g.IdSkimPerkhidmatan == a.Id
                                                   select g.IdGred.ToString()).ToList(),
                                       ketuList = (from h in _dbContext.PDOSkimKetuaPerkhidmatan
                                                   where h.IdSkimPerkhidmatan == a.Id
                                                   select h.IdKetuaPerkhidmatan.ToString()).ToList(),
                                       KodRujJenisSaraan = a.KodRujJenisSaraan,
    ...
    .ToListAsync();
                    ...
                    var idJawtanList = dto.IdJawatan
                            .Split(',', StringSplitOptions.RemoveEmptyEntries)
                            .Select(id => Convert.ToInt32(id.Trim()))
                            .ToList();
        ...
        public async Task<bool> DaftarHantarSkimPerkhidmatanAsync(MaklumatSkimPerkhidmatanCreateRequestDto dto)
        {
            ...
            try
            {
                ...
                if (idPerkhidmatan == 0)
                {
                    ...
                    // Step 4: Insert PDO_SkimKetuaPerkhidmatan
                    if (!string.IsNullOrEmpty(dto.IdJawatan))
                    {
                        var idJawatanList = dto.IdJawatan
                            .Split(',', StringSplitOptions.RemoveEmptyEntries)
                            .Select(id => Convert.ToInt32(id.Trim()))
                            .ToList();

                        foreach (var idJawatan in idJawatanList)
                        {
    ...
    .ToListAsync();
        ...
        public async Task<bool> KemaskiniStatusAsync(SkimPerkhidmatanRefStatusDto perkhidmatanDto)
        {
            ...
            try
            {
                ...
                // Step 5: Insert into PDO_StatusPermohonanSkimPerkhidmatan
                var statusEntity = new PDOStatusPermohonanSkimPerkhidmatan
                {
